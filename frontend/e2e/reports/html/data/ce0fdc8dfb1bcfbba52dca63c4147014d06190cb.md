...
- Name: routes.spec.ts >> Route Protection >> should redirect to login when accessing protected route without token
- Location: e2e\tests\routes.spec.ts:65:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:5173/auth/login
Call log:
...
> 69  |     await page.goto("/auth/login");
  ...
  133 | 
  134 |     // Set auth token
  135 |     await setAuthToken(page, registered.token);
  136 | 
  137 |     // Navigate to settings
  138 |     await page.goto("/settings");
  139 |     await page.waitForLoadState("networkidle");
  140 | 
  141 |     // Should be on settings page
  142 |     expect(page.url()).toContain("/settings");
  143 | 
  144 |     // Cleanup
  145 |     await cleanupUserData(registered.token);
  146 |   });
  147 | });
  148 | 
```